...
def plot_diagrams():
    ...
    plt.fill_betweenx(
        P_real, x1_range, y1_calc, color="gray", alpha=0.2, label="Двухфазная область"
    )
...
if __name__ == "__main__":
    ...
    gamma1 = np.exp(
        -np.log(x1_range + lambda12 * x2_range)
        + x2_range
        * (
            lambda12 / (x1_range + lambda12 * x2_range)
            - lambda21 / (lambda21 * x1_range + x2_range)
        )
    )
    gamma2 = np.exp(
        -np.log(x2_range + lambda21 * x1_range)
        - x1_range
        * (
            lambda12 / (x1_range + lambda12 * x2_range)
            - lambda21 / (lambda21 * x1_range + x2_range)
